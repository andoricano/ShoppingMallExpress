...
* [x] Admin Refund reads and `APPROVED`/`REJECTED` transitions use the server-only boundary and `admin_transition_refund_status()`.
  * [x] Refund restock (Phase 8 policy B) uses the internal `refund_item_restocks` table and the service-role-only `admin_restock_refund_item(uuid, uuid, uuid, bigint)` RPC (migration `20260925150000_refund_item_restocks.sql`; `supabase/verification/refund_item_restocks.sql` and `refund_item_restocks_concurrency.sh`) through `POST /api/admin/refunds/[refundId]/restocks`. `refund_item_restocks` has RLS enabled, no policy, and no `anon`/`authenticated` privilege. The RPC requires an `APPROVED` refund, a refund item of that refund, and an original `order_item_ware_allocations` Ware; it locks the refund item, the allocation and the Ware, and keeps restocked quantity within the refunded quantity per refund item and within the Ware allocation per (OrderItem, Ware). The Admin Refund screen shows refunded/restocked/remaining quantities and offers only the OrderItem's original allocation Wares. The same migration stamps `refund_requests.processed_at` on `APPROVED`/`REJECTED`.
  * [x] `PATCH /api/admin/refunds/[refundId]` sent `p_next_status` to `admin_transition_refund_status`, whose applied parameter is `p_status`, so every Admin approve/reject failed with PostgREST `PGRST202`. The route now sends `p_status`.
...
* [x] **Removed:** `apps/develop-web` (internal development app that called the legacy Express API) was deleted in `8ef3741`.
...
### Stock expectations (Refund policy B)

* Order creation deducts the allocated Ware `current_stock`; Cancel (`PENDING` only) restores the exact allocated Ware stock.
* Refund `APPROVED` (and `REQUESTED`/`REJECTED`) never changes Ware stock: **stock is unchanged after Refund `APPROVED`**.
