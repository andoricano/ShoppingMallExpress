...
* [ ] Order, Wishlist, History, Refund, and Point/Payment still call the removed Express API. The cart page's order button stays disabled, and `store/cartStore.ts` (legacy item shape) remains only for the Order screens until the Order migration. The detail page's cart action still uses the legacy `useCart.addCart(productId, quantity)`, and "구매하기" stays disabled until the v2 Order flow is connected.
