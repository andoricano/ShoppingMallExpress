-- ============================================================
-- Mall v2
-- Extend admin_update_product(): Product image_urls
--
-- Replaces admin_update_product(uuid, jsonb, jsonb, jsonb) from
-- 20260925100000_admin_update_product_structure.sql with the SAME
-- signature and return type. All existing semantics are unchanged;
-- this only adds, in the same single transaction:
--
--   p_product.imageUrls  ordered array of absolute http(s) URLs that
--                        REPLACES products.image_urls ([] clears it).
--
-- An invalid imageUrls value (not an array, a non-string / blank
-- element, or a non-http(s) URL such as a temporary blob: URL)
-- raises and rolls back the whole update, so no partial image list
-- is ever stored. image_urls keeps the create contract of
-- admin_create_product() (text[], in display order).
--
-- Input (only present keys change):
--   p_product  {name?, description?, isActive?, imageUrls?}
--   p_options / p_variants: unchanged, see
--   20260925100000_admin_update_product_structure.sql
-- ============================================================
...
begin
    ...
    if p_product ? 'imageUrls' then
        if jsonb_typeof(p_product -> 'imageUrls') <> 'array' then
            raise exception 'Product imageUrls must be an array';
        end if;

        if exists (
            select 1
            from jsonb_array_elements(p_product -> 'imageUrls') as image(url)
            where jsonb_typeof(image.url) <> 'string'
               or (image.url #>> '{}') !~ '^https?://[^[:space:]]+$'
        ) then
            raise exception
                'Product imageUrls must contain only absolute http(s) URLs';
        end if;
    end if;
    ...
    update public.products p
       set name = case when p_product ? 'name'
           ...
           image_urls = case when p_product ? 'imageUrls'
                             then array(
                                 select image.url
