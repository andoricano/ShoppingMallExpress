#!/usr/bin/env bash
# ============================================================
# Local verification: Mall v3 Consumer checkout flow (apps/client-web/lib/checkoutFlow.ts) over HTTP
#
#   bash supabase/verification/v3_phase8_checkout_flow.sh
#
...
# Fixtures are committed (names start with "__v3p8flow", users are
# v3p8-*@example.test) and removed on exit; `pnpm supabase db reset` restores
...
WH="" POST="" PRODUCT_NAME="__v3p8flow product"
cleanup() {
...
create temp table _u as select id from auth.users where email like 'v3p8-%@example.test';
...
WH="$(sql "select public.create_warehouse('__v3p8flow warehouse')")"
W1="$(sql "select public.create_ware('$WH', '__v3p8flow W1', null, 'GENERAL', 100)")"
PJ="$(sql "select public.admin_create_product('{\"name\":\"$PRODUCT_NAME\"}', '[]', '[{\"price\":1000,\"skuCode\":\"__V3P8F-1\"},{\"price\":500,\"skuCode\":\"__V3P8F-2\"},{\"price\":100,\"skuCode\":\"__V3P8F-Z\"}]')")"
PRODUCT="$(sql "select ('$PJ'::jsonb ->> 'productId')")"
V1="$(sql "select ('$PJ'::jsonb -> 'variantIds' ->> 0)")"
V2="$(sql "select ('$PJ'::jsonb -> 'variantIds' ->> 1)")"
VZ="$(sql "select ('$PJ'::jsonb -> 'variantIds' ->> 2)")"
psqlq -c "select public.link_product_variant_ware('$V1','$W1')" >/dev/null
POST="$(sql "insert into public.product_posts (title, status) values ('__v3p8flow post','PUBLISHED') returning id")"
psqlq -c "insert into public.product_post_products (product_post_id, product_id) values ('$POST','$PRODUCT')" >/dev/null

FX="{\"product\":\"$PRODUCT\",\"v1\":\"$V1\",\"v2\":\"$V2\",\"vz\":\"$VZ\",\"w1\":\"$W1\"}"

# The Admin route (apps/user-web) uses the same executor: the two copies must be identical.
if ! cmp -s "$ROOT/apps/client-web/lib/payment/reversal.ts" "$ROOT/apps/user-web/lib/payment/reversal.ts"; then
  echo "FAIL  apps/user-web/lib/payment/reversal.ts differs from apps/client-web/lib/payment/reversal.ts"
  exit 1
fi
...
cd "$ROOT" && LOCAL_API_URL="$LOCAL_API_URL" LOCAL_ANON_KEY="$LOCAL_ANON_KEY" LOCAL_SERVICE_KEY="$LOCAL_SERVICE_KEY" \
  FX="$FX" node --no-warnings supabase/verification/v3_phase8_checkout_flow.mts
