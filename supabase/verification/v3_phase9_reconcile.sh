#!/usr/bin/env bash
# ============================================================
# Local verification: Mall v3 reconcile job (apps/client-web/app/api/internal/reconcile) over HTTP
#
#   bash supabase/verification/v3_phase9_reconcile.sh
#
...
# Fixtures are committed (names start with "__v3p9recon", users are
# v3p9-*@example.test) and removed on exit; `pnpm supabase db reset` restores
...
WH="" POST="" PRODUCT_NAME="__v3p9recon product"
cleanup() {
...
create temp table _u as select id from auth.users where email like 'v3p9-%@example.test';
...
WH="$(sql "select public.create_warehouse('__v3p9recon warehouse')")"
W1="$(sql "select public.create_ware('$WH', '__v3p9recon W1', null, 'GENERAL', 100)")"
WS="$(sql "select public.create_ware('$WH', '__v3p9recon WS', null, 'GENERAL', 10)")"
...
POST="$(sql "insert into public.product_posts (title, status) values ('__v3p9recon post','PUBLISHED') returning id")"
...
cd "$ROOT" && LOCAL_API_URL="$LOCAL_API_URL" LOCAL_ANON_KEY="$LOCAL_ANON_KEY" LOCAL_SERVICE_KEY="$LOCAL_SERVICE_KEY" \
  FX="$FX" node --no-warnings supabase/verification/v3_phase9_reconcile.mts
