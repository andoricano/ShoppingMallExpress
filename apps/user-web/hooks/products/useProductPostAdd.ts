...
export function useProductPostAdd() {
    ...
    const createProductPost = useCallback(async () => { setSaving(true); setError(null); try { let thumbnailUrl = draftPost.thumbnailUrl; if (thumbnailFile) { const uploaded = await uploadImage(thumbnailFile); thumbnailUrl = uploaded.imageUrl; } const response = await fetch("/api/admin/product-posts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...draftPost, thumbnailUrl, productIds: draftProducts.map((item) => item.id) }) }); if (!response.ok) throw new Error("상품 게시물 저장에 실패했습니다."); } catch (cause) { setError(cause instanceof Error ? cause.message : "상품 게시물 저장에 실패했습니다."); throw cause; } finally { setSaving(false); } }, [draftPost, draftProducts, thumbnailFile, uploadImage]);
